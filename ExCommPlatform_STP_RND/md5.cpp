...
void md5_ful27(const char *message, uint32_t len, uint32_t hash[4])
{
  ...
  {
		md5_com91(hash, (uint32_t *)(message + i));  // Type-punning
  }
...
void md5_inc11L(const char *message, uint32_t hash[4], char* chLogBuff)
{
  hash[0] = 0x67452301;
	...
	hash[2] = 0x98BADCFE;
	hash[3] = 0x10325476;
  md5_com91(hash, (uint32_t *)message);
  sprintf(chLogBuff, "A %0X|B %0X|C %0X|D %0X|md5_inc11L|", hash[0], hash[1], hash[2], hash[3]);
}
