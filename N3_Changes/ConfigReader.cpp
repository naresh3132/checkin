...
void ConfigReader::parseFile()
{
  ...
  if(configFile.is_open())
  {
    while(getline(configFile, line))
    {
      ...
      if((pos=line.find_first_of(_delimiter))!=std::string::npos)
      {
...
ConfigReader::ConfigReader(std::string filename,const char delimiter):_filename(filename),_delimiter(delimiter)
{
  parseFile();  
};
