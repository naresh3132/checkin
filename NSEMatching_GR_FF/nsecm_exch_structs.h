...
//    char Timestamp[8];
    ...
    inline void swapBytes()
    {
      TransactionCode      = __bswap_16(TransactionCode);
      LogTime      = __bswap_32(LogTime);
      iTraderId      = __bswap_32(iTraderId);
      ErrorCode      = __bswap_16(ErrorCode);
      Timestamp      = __bswap_64(Timestamp);
      Timestamp1      = __bswap_64(Timestamp1);
      MessageLength      = __bswap_16(MessageLength);   
...
//NEW LOGIN CHANGES ENDS
  ...
  //  inline void Initialize(char* pchData)
  //  {
  //    memcpy(&Header.TransactionCode, pchData, sizeof(MS_SIGNON_RESP));
  //
  //    Header.TransactionCode = __bswap_16(Header.TransactionCode);
  //    Header.ErrorCode = __bswap_16(Header.ErrorCode);
  //  }
...
//    TAP_HEADER tap_hdr;
  ...
  /*TO HANDLE SL TRIGGERED ORDER(BECAUSE TAP HEADER IS NOT INCLUDED IN MS_TRADE_CONFIRM)*/
...
typedef MS_SL_TRIGGER NSE_CM_MS_SL_TRIGGER;
  ...
  */
  ...
  struct MS_OE_RESPONSE_TR
  {
    ...
    {
        TransactionCode = __bswap_16(TransactionCode);
        int32_t LogTime = __bswap_32(LogTime);
        int32_t TraderId = __bswap_32(TraderId);
        int16_t ErrorCode = __bswap_16(ErrorCode);
        int64_t TimeStamp1 = __bswap_64(TimeStamp1); 
        ...
        int32_t DisclosedVolume = __bswap_32(DisclosedVolume);
        int32_t DisclosedVolumeRemain = __bswap_32(DisclosedVolumeRemain) ;
        int32_t TotalVolumeRemain = __bswap_32(TotalVolumeRemain);
        int32_t Volume = __bswap_32(Volume);
        int32_t VolumeFilledToday = __bswap_32(VolumeFilledToday);
        int32_t Price = __bswap_32(Price);
        int32_t EntryDateTime = __bswap_32(EntryDateTime);
        int32_t LastModified = __bswap_32(LastModified);
        int16_t BranchId = __bswap_16(BranchId);
        int32_t UserId = __bswap_32(UserId);  
        int16_t ProClient = __bswap_16(ProClient);
        int16_t SettlementPeriod __bswap_16(SettlementPeriod);
        double NnfField = SwapDouble(NnfField);  
        int32_t TransactionId = __bswap_32(TransactionId);  
    }*/
  };
