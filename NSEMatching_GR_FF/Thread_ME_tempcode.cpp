...
void StartME(ProducerConsumerQueue<_DATA_RECEIVED>* Inqptr_TCPServerToMe,
             ...
             int32_t* Tokenarr, int TokenCnt, dealerInfoMap* dealerInfomap, CONTRACTINFO* pCntrctInfo)
{
    TaskSetCPU(iMECore);
...
int SendOrderCancellation_NSECM(ORDER_BOOK_DTLS *orderBook, long Token,int FD,CONNINFO* pConnInfo, int COL, bool sendBrdcst)
{
    ...
    memcpy(&bookdetails, orderBook, sizeof(bookdetails));
        snprintf(logBuf, 500, "Order# %ld|TradeID %d|Booktype %d|Buy/Sell %d|Volume %d|BranchId %d|UserId %d|ProClientInd %d|TransactionId %d|Suspended %c| NnfField %f|AccountNumber %s|BrokerId %s|Settlor %s|symbol %s|series %s|Day %c| IOC %c",
       long(bookdetails.OrderNo), bookdetails.TraderId, bookdetails.BookType, bookdetails.BuySellIndicator, bookdetails.Volume, bookdetails.BranchId, bookdetails.UserId,
      bookdetails.ProClientIndicator, bookdetails.nsecm_nsefo.NSECM.TransactionId, bookdetails.nsecm_nsefo.NSECM.Suspended, bookdetails.NnfField,
      bookdetails.AccountNumber, bookdetails.BrokerId, bookdetails.Settlor, bookdetails.nsecm_nsefo.NSECM.sec_info.Symbol, 
      bookdetails.nsecm_nsefo.NSECM.sec_info.Series, bookdetails.nsecm_nsefo.NSECM.OrderFlags.Day,
      bookdetails.nsecm_nsefo.NSECM.OrderFlags.IOC);
    Logger::getLogger().log(DEBUG, logBuf);
        /*temp code*/
...
long Addtoorderbook(ORDER_BOOK_DTLS * Mybookdetails, int BuySellSide, long Token,int IsIOC,int IsDQ,int IsSL, int32_t epochTime, int dealerIndx, int tokenIndx ) // 1 Buy , 2 Sell
{
    ...
    /*temp code*/
           int indx = ME_OrderBook.OrderBook[tokenIndx].BuyRecords -1;
           ORDER_BOOK_DTLS bookdetails;
             memcpy(&bookdetails, &(ME_OrderBook.OrderBook[tokenIndx].Buy[indx]), sizeof (bookdetails));
    snprintf(logBuf, 500, "Order# %ld|TradeID %d|Booktype %d|Buy/Sell %d|Volume %d|BranchId %d|UserId %d|ProClientInd %d|TransactionId %d|Suspended %c| NnfField %f|AccountNumber %s|BrokerId %s|Settlor %s|symbol %s|series %s|Day %c| IOC %c",
       long(bookdetails.OrderNo), bookdetails.TraderId, bookdetails.BookType, bookdetails.BuySellIndicator, bookdetails.Volume, bookdetails.BranchId, bookdetails.UserId,
      bookdetails.ProClientIndicator, bookdetails.nsecm_nsefo.NSECM.TransactionId, bookdetails.nsecm_nsefo.NSECM.Suspended, bookdetails.NnfField,
      bookdetails.AccountNumber, bookdetails.BrokerId, bookdetails.Settlor, bookdetails.nsecm_nsefo.NSECM.sec_info.Symbol, 
    ...
    else
    {
        ...
        // Start Handling DQ Order -------------------------------------------------   
          ...
          ME_OrderBook.OrderBook[tokenIndx].SellRecords = ME_OrderBook.OrderBook[tokenIndx].SellRecords + 1;
                        /*temp code*/
           int indx = ME_OrderBook.OrderBook[tokenIndx].SellRecords -1;
           ORDER_BOOK_DTLS bookdetails;
              memcpy(&bookdetails, &(ME_OrderBook.OrderBook[tokenIndx].Sell[indx]), sizeof (bookdetails));
    snprintf(logBuf, 500, "Order# %ld|TradeID %d|Booktype %d|Buy/Sell %d|Volume %d|BranchId %d|UserId %d|ProClientInd %d|TransactionId %d|Suspended %c| NnfField %f|AccountNumber %s|BrokerId %s|Settlor %s|symbol %s|series %s|Day %c| IOC %c",
       long(bookdetails.OrderNo), bookdetails.TraderId, bookdetails.BookType, bookdetails.BuySellIndicator, bookdetails.Volume, bookdetails.BranchId, bookdetails.UserId,
      bookdetails.ProClientIndicator, bookdetails.nsecm_nsefo.NSECM.TransactionId, bookdetails.nsecm_nsefo.NSECM.Suspended, bookdetails.NnfField,
      bookdetails.AccountNumber, bookdetails.BrokerId, bookdetails.Settlor, bookdetails.nsecm_nsefo.NSECM.sec_info.Symbol, 
...
int ModOrderTrim(NSECM::MS_OM_REQUEST_TR *ModOrder,int FD,int IsIOC,int IsDQ,int IsSL,CONNINFO* pconnInfo )
{
    ...
    if (ModOrdResp.ErrorCode != 0)
    {
         snprintf(logBuf, 500, "Thread_ME|FD %d|MOD ORDER|Order# %ld|COrd# %d|ErrorCode %d|Symbol %s|Series %s|Token %ld|LMT %d",
           FD,int(dOrderNo), ModOrder->TransactionId,ModOrdResp.ErrorCode,ModOrder->sec_info.Symbol,ModOrder->sec_info.Series,Token, LMT);
         Logger::getLogger().log(DEBUG, logBuf);
...
int ModOrderTrim(NSEFO::MS_OM_REQUEST_TR *ModOrder,int FD,int IsIOC,int IsDQ,int IsSL,CONNINFO* pConnInfo )
{
    ...
    if (ModOrdResp.ErrorCode != 0)
    {
        snprintf (logBuf, 500, "Thread_ME|FD %d|MOD ORDER|Order# %ld|COrd# %d|ErrorCode %d|Token %ld|LMT %d", 
          FD, int(dOrderNo),ModOrder->filler,ModOrdResp.ErrorCode,(Token + FOOFFSET), LMT);
        Logger::getLogger().log(DEBUG, logBuf);
...
int CanOrderTrim(NSECM::MS_OM_REQUEST_TR *ModOrder,int FD, CONNINFO* pConnInfo)
{
    ...
    if (CanOrdResp.ErrorCode != 0)
    {
        snprintf (logBuf, 500, "Thread_ME|FD %d|CAN ORDER|Order# %ld|COrd# %d|ErrorCode %d|Symbol %s|Series %s|Token %ld|LMT",
          FD,int(dOrderNo), ModOrder->TransactionId,CanOrdResp.ErrorCode,ModOrder->sec_info.Symbol,ModOrder->sec_info.Series,Token, LMT);
        Logger::getLogger().log(DEBUG, logBuf);
    ...
    FD, int (bookdetails.OrderNo), ModOrder->TransactionId,bookdetails.lQty, bookdetails.lPrice, Token, __bswap_16(CanOrdResp.BuySellIndicator), LMT);
    Logger::getLogger().log(DEBUG, logBuf);
...
int CanOrderTrim(NSEFO::MS_OM_REQUEST_TR *ModOrder,int FD, CONNINFO* pConnInfo)
{
    ...
    if (CanOrdResp.ErrorCode != 0)
    {
        snprintf (logBuf, 500, "Thread_ME|FD %d|CAN ORDER|Order# %ld|COrd# %d|ErrorCode %d|Token %ld|LMT %d",
          FD,int(dOrderNo), ModOrder->filler,CanOrdResp.ErrorCode,(Token + FOOFFSET), LMT);
        Logger::getLogger().log(DEBUG, logBuf);
    ...
    snprintf (logBuf, 500, "Thread_ME|FD %d|CAN ORDER|Order# %ld|COrd# %d|Qty %ld|Price %ld|Token %ld|Side %d|LMT %d",
    FD, int (bookdetails.OrderNo), ModOrder->filler,bookdetails.lQty, bookdetails.lPrice, (Token + FOOFFSET), __bswap_16(CanOrdResp.BuySellIndicator),
      LMT);
    Logger::getLogger().log(DEBUG, logBuf);
        ...
        {   
             if(ME_OrderBook.OrderBook[tokenIndex].Buy[j].OrderNo == Mybookdetails->OrderNo)
             {
                ...
                memcpy(Mybookdetails, &(ME_OrderBook.OrderBook[tokenIndex].Buy[j]), sizeof(Mybookdetails));
                                        ...
                                        /*temp code*/
            ORDER_BOOK_DTLS bookdetails;
           memcpy(&bookdetails, &(ME_OrderBook.OrderBook[tokenIndex].Buy[j]), sizeof (bookdetails));
    snprintf(logBuf, 500, "Order# %ld|TradeID %d|Booktype %d|Buy/Sell %d|Volume %d|BranchId %d|UserId %d|ProClientInd %d|TransactionId %d|Suspended %c| NnfField %f|AccountNumber %s|BrokerId %s|Settlor %s|symbol %s|series %s|Day %c| IOC %c",
       long(bookdetails.OrderNo), bookdetails.TraderId, bookdetails.BookType, bookdetails.BuySellIndicator, bookdetails.Volume, bookdetails.BranchId, bookdetails.UserId,
      bookdetails.ProClientIndicator, bookdetails.nsecm_nsefo.NSECM.TransactionId, bookdetails.nsecm_nsefo.NSECM.Suspended, bookdetails.NnfField,
      bookdetails.AccountNumber, bookdetails.BrokerId, bookdetails.Settlor, bookdetails.nsecm_nsefo.NSECM.sec_info.Symbol, 
        ...
        {   
             if(ME_OrderBook.OrderBook[tokenIndex].Sell[j].OrderNo == Mybookdetails->OrderNo)
             {
                ...
                memcpy (Mybookdetails, &(ME_OrderBook.OrderBook[tokenIndex].Sell[j]), sizeof(Mybookdetails));
                                        /*temp code*/
           ORDER_BOOK_DTLS bookdetails;
           memcpy(&bookdetails, &(ME_OrderBook.OrderBook[tokenIndex].Sell[j]), sizeof (bookdetails));
    snprintf(logBuf, 500, "Order# %ld|TradeID %d|Booktype %d|Buy/Sell %d|Volume %d|BranchId %d|UserId %d|ProClientInd %d|TransactionId %d|Suspended %c| NnfField %f|AccountNumber %s|BrokerId %s|Settlor %s|symbol %s|series %s|Day %c| IOC %c",
       long(bookdetails.OrderNo), bookdetails.TraderId, bookdetails.BookType, bookdetails.BuySellIndicator, bookdetails.Volume, bookdetails.BranchId, bookdetails.UserId,
      bookdetails.ProClientIndicator, bookdetails.nsecm_nsefo.NSECM.TransactionId, bookdetails.nsecm_nsefo.NSECM.Suspended, bookdetails.NnfField,
      bookdetails.AccountNumber, bookdetails.BrokerId, bookdetails.Settlor, bookdetails.nsecm_nsefo.NSECM.sec_info.Symbol, 
                ...
                // ---- Sell Handling 
                 ...
                 switch(_nSegMode)
                 {
                   case SEG_NSECM: //Order_Store_NSECM
                   {
                     ...
                     snprintf (logBuf, 500, "Thread_ME|FD %d|Sell Trade|Order# %ld|Qty %d|Price %ld|Token %ld|VFT %d|LMT %d",
                       sellFD,orderNo,TradeQty,TradePrice, Token, __bswap_32(SendTradeConf.VolumeFilledToday), __bswap_32 (SendTradeConf.ActivityTime));
                      Logger::getLogger().log(DEBUG, logBuf);
                     ...
                     snprintf (logBuf, 500, "Thread_ME|FD %d|Buy Trade|Order# %ld|Qty  %d|Price %ld|Token %ld|VFT %d|LMT %d",
                     buyFD,orderNo,TradeQty,TradePrice, Token, __bswap_32(SendTradeConf.VolumeFilledToday), __bswap_32(SendTradeConf.ActivityTime));
                     Logger::getLogger().log(DEBUG, logBuf);
